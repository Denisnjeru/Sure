...
def aws_text_detection(document):
    ...
    aws_key = config('AWS_SECRET_ACCESS_KEY')

    #Get the document from S3
    s3_connection = boto3.resource('s3', region_name=region, aws_access_key_id=key_id, aws_secret_access_key=aws_key)

    s3_object = s3_connection.Object(bucket,document)
    s3_response = s3_object.get()

    # Detect text in the document
    client = boto3.client('textract', region_name=region, aws_access_key_id=key_id, aws_secret_access_key=aws_key)
...
def upload_file(file_path):
    ...
    key = folder + '/' + os.path.basename(file_path)
    # Upload the file
    s3_client = boto3.client('s3', region_name=region, aws_access_key_id=key_id, aws_secret_access_key=aws_key)
    print(bucket)
    print(key)
    print(file_path)
    try:
...
def verify_tcc(document=None, local_path=None):
    """
    ...
    # initialize variable to store text extracted from document
    text = ""
    expiry_date = None
    kra_pin = None
    company_name = None
    ...
    if local_path != None:
        document = upload_file(local_path)
    ...
    try:
        company_name = re.search(r'This is to confirm that ([\S\s]*?)Personal Identification', text).group(1)
...
def verify_cr12(document=None, local_path=None):
    # initialize variable to store text extracted from document
    text = ""
    company_name = None
    cr12_ref = None
...
def verify_nca(document=None, local_path=None):
    # initialize variable to store text extracted from document
    text = ""
    expiry_date = None
    serial_no = None
...
def verify_nationalid(document=None, local_path=None):
    # initialize variable to store text extracted from document
    text = ""
    names = None
    id_no = None
...
def verify_businesspermit(document=None, local_path=None):
    # initialize variable to store text extracted from document
    text = ""
    business_name = None
    business_id = None
...
def verify_poisonsboardcert(document=None, local_path=None):
    # initialize variable to store text extracted from document
    text = ""
    expiry_date = None
    company_name = None
...
def verify_pincert(document=None, local_path=None):
    # initialize variable to store text extracted from document
    text = ""
    pin = None
...
def verify_incorporationcert(document=None, local_path=None):
    # initialize variable to store text extracted from document
    text = ""
    name = None
    date = None
