...
from apps.prequal.models import Prequalification
...
def email_category_notifications(data, files, category_id):
    ...
    category = apps.apps.get_model('prequal', 'Category').objects.filter(
        id=category_id
    ).prefetch_related('prequalification').first()
...
def send_email_notification(bidder, subject, text, files=None, old=None):
    try:
        message = render_to_string(
            "prequal/emails/supplier/notification.html",
    ...
    try:
        if country not in countries:
            body = render_to_string(
                "prequal/emails/supplier/non_responsive_remainder_count.html",
                {
            ...
            body = render_to_string(
                "prequal/emails/supplier/custom/non_responsive_remainder_count.html",
                {
                    "supplier": bidder.company_name, "company": company,
                    "category": category,"Date": category.closing_date.date(),
                    "Time": category.closing_date.time().strftime("%H:%M"),
...
def send_potential_email_extension_notification(suppliers, category):
    ...
    if len(suppliers) >= 99:
        ...
        try:
            if country not in countries:
                body = render_to_string(
                    "prequal/emails/supplier/potential_extension.html",
                    {
                ...
                help_contacts = apps.apps.get_model('core', 'Helpcontacts').objects.get(country=country)
                body = render_to_string(
                    "prequal/emails/supplier/custom/potential_extension.html",
                    {
                        "supplier": "Esteemed Vendor", "company": company,
                        "category": category.name, "Date": category.closing_date.date(),
                        "Time": category.closing_date.time().strftime("%H:%M"),
    ...
    if len(suppliers) >= 99:
        ...
        for split_supplier in split_suppliers:
            ...
            try:
                if country not in countries:
                    body = render_to_string(
                        "prequal/emails/supplier/potential_remainder_count.html",
                        {
                            "supplier": "Esteemed Vendor", "company": company,
                            "category": category.name, "Date": category.closing_date.date(),
                            "Time": category.closing_date.time().strftime("%H:%M"),
                    ...
                    help_contacts = apps.apps.get_model('core', 'Helpcontacts').objects.get(country=country)
                    body = render_to_string(
                        "prequal/emails/supplier/custom/potential_remainder_count.html",
                        {
                            "supplier": "Esteemed Vendor", "company": company, "category": category.name,
                            "Date": category.closing_date.date(),
                            "Time": category.closing_date.time().strftime("%H:%M"),
...
def default_email(potential_suppliers_emails, category):
    ...
    if len(potential_suppliers_emails) >= 99:
        ...
        try:
            if country not in countries:
                body = render_to_string(
                    "prequal/emails/supplier/supplier_invite.html",
                    {"supplier": "Esteemed Vendor", "company": company,
                ...
                help_contacts = apps.apps.get_model('core', 'Helpcontacts').objects.get(country=country)
                body = render_to_string(
                    "prequal/emails/supplier/custom/supplier_invite.html",
                    {
                        "supplier": "Esteemed Vendor", "company": company, "category": category.name,
                        "phone": help_contacts.contact_phone, "help_email": help_contacts.helpemail,
                    },
...
# job notifications start here
@shared_task()
def initiate_email_out_sending(job_id):
    job = apps.apps.get_model("prequal", "Prequalification").objects.get(id=job_id)
    ...
    for category in categories:
        ...
        emails = apps.apps.get_model("prequal", "EmailOut").objects.filter(
            category_id=category.id, sent=False, type=apps.apps.get_model("prequal", "EmailOut").REMINDER, has_error=False,
            created_at__year=date.year, created_at__month=date.month, created_at__day=date.day, 
            target__model="prequalification"
        ).order_by('-id')
        c = emails.count()

        while c > 0:
            to_send = apps.apps.get_model("prequal", "EmailOut").objects.filter(
                category_id=category.id, sent=False, type=apps.apps.get_model("prequal", "EmailOut").REMINDER, has_error=False,
                created_at__year=date.year, created_at__month=date.month, created_at__day=date.day,
                target__model="prequalification"
            ).order_by('-id')[:90]
            ...
            try:
                ...
                email.send()
                apps.apps.get_model("prequal", "EmailOut").objects.filter(id__in=ids).update(sent=True)
            except Exception as e:
                capture_exception(e)
                apps.apps.get_model("prequal", "EmailOut").objects.filter(id__in=ids).update(error=f"{e}", has_error=True)
            c -= 90
    return


class SendEmailNotifications(Task):
                ...
                if supplier != "" and supplier != " " and supplier is not None:
                    try:
                        emails_to_store.append(
                            apps.apps.get_model("core", "EmailOut")(
                                subject=email_subject,
                                to=supplier, message=message, body=body, category_id=category.id,
                                type=apps.apps.get_model("core", "EmailOut").REMINDER,
                                target__model="prequalification"
                            )
                        )
                        if len(emails_to_store) >= 90:
                            apps.apps.get_model("core", "EmailOut").objects.bulk_create(emails_to_store)
            ...
            for bidder in bidders:
                try:
                    if country not in countries:
                        ...
                        # send with  personalized phone and email
                        help_contacts = apps.apps.get_model("prequal", "HelpContact").objects.get(country=country)
                        body = render_to_string(
...
@shared_task()
def send_job_invite_email(job_id):
    try:
        ...
        for category in categories:
            ...
            if category.invite_notification == False:
                if category.invite_only == True:
                    ...
                    """
                    company = job.company
                    suppliers = category.category_type.suppliers_list(country=company.country)
                        ...
                        for split_old_supplier in split_old_suppliers:
                            ...
                            if country not in countries:
                                try:
                                    ...
                                    time = datetime.datetime.now()
                                    file_url = A.url(
                                        f"{job.advert}", expire=300, parameters=headers, http_method="GET",)
                                        ...
                                        for supplier_email in split_old_supplier:
                                            if Supplier.objects.filter(email=supplier_email).exists():
                                                if Supplier.objects.filter(email=supplier_email).exists():
                                                    ...
                                                    obj, created = apps.apps.get_model("prequal", "SupplierBuyerNotification").objects.update_or_create(
                                                        supplier=supplier, buyer=buyer, job=job, email_subject=email_subject,
...
@shared_task()
def send_job_invite_sms(job_id):
...
def send_participants_email(category_id):
    ...
    try:
        ...
        template_path = "prequal/emails/supplier/list_of_participants.html"
        context = {
        ...
        for participant in participants:
            ...
            try:
                ...
                body = render_to_string(
                    "prequal/emails/supplier/list_of_participants_body.html",
                    {
        ...
        body = render_to_string(
            "prequal/emails/supplier/list_of_participants_body.html", mail_context
        )
...
def broadcast_job_notifications(self, data):

    job = apps.apps.get_model('prequal', 'Prequalification').objects.get(id=data["job_id"])
    level = data["level"]
    verb = data["verb"]
        ...
        for bidder in category.paid_bidders:
            new_notification = apps.apps.get_model("core", "Notifications").objects.create(
                ...
                action_object_content_type=ContentType.objects.get(
                    app_label="prequal", model="category"
                ),
...
class SendSMSNotifications(Task):
    ...
    def run(self, *args, **kwargs):
        ...
        potential_selection = kwargs["data"]["potential_selection"]
        type_class = kwargs["data"]["type"]
        categories = apps.apps.get_model("prequal", "Category").objects.filter(job_id=kwargs["data"]['job_id'])

        if type_class == "potentialsms":
            if potential_selection == 'Defaultsms':
                print("default sms")
                self.send_potential_default_sms(categories)
            else:
                print("Not default sms")
        else:
            print("Not potential sms")

    def send_potential_default_sms(self, categories):
        category_types = set()
        messages = []
        company = categories.first().prequalification.company
        country = company.country
        for category in categories:
            category_type = category.category_type
            if category_type not in category_types:
                if category.paid_bidders:
                    potential_suppliers = self.calculate_potential_suppliers(
                        category.paid_bidders, category
                    )
                elif len(category.paid_bidders) == 0:
                    category_type_suppliers = []
                    potential_dictionary = {}
                    potential_dictionary = category_type.suppliers_list(country)
                    category_type_suppliers.extend(potential_dictionary["registered_suppliers"])
                    category_type_suppliers.extend(potential_dictionary["old_suppliers"])
                    potential_suppliers = category_type_suppliers
                else:
                ...
                try:
                    if potential_suppliers:
                        sms_out = []
                        phone_numbers = []
                        m = f"Tendersure is managing the prequalification for {company}. Closing date " \
                            f"{category.closing_date.date()}.To bid go to www.tendersure.co.ke and click 'Available " \
                            f"Jobs'."
                        for potential_supplier in potential_suppliers:
                            if potential_supplier:
                                if potential_supplier._meta.model.__name__ == "CategoryTypeSupplier":
                                    supplier_phone = potential_supplier.primary_phone
                                    phone_numbers.append(supplier_phone)
                                else:
                                    supplier_phone = potential_supplier.phone_number
                                    phone_numbers.append(supplier_phone)

                        for phone_number in list(set(phone_numbers)):
                            sms_out.append(
                                apps.apps.get_model("prequal", "SmsOut")(
                                    category_id=category.id, phone_number=phone_number,
                                    text_message=m,
                                    unique_identifier=f"{category.unique_reference}" + f"{random.randint(1000, 999999)}")
                            )
                            if len(phone_numbers) >= 100:
                                apps.apps.get_model("prequal", "SmsOut").objects.bulk_create(sms_out)
                                sms_out.clear()
                        apps.apps.get_model("prequal", "SmsOut").objects.bulk_create(sms_out)
                        sms_out.clear()
                    ...
                    capture_exception(e)
                    messages.append(
                        f"Sms notification to category {category.name} were not sent!")
            else:
                messages.append("Sms to this category type have already been sent !")
        context = {
            "response_message": "Sms sending complete",
            "messages": messages
        }
        return context

    def calculate_potential_suppliers(self, paid_bidders, category):
        paid_bidders_suppliers = []
        potential = []
        ...
        category_type_suppliers = category.category_type.suppliers_list_emails(
            country=category.job.company.country
        )
        category_type_suppliers_suppliers = list(
            category_type_suppliers["old_suppliers"])
        registered_suppliers = category_type_suppliers["registered_suppliers"]
        category_type_suppliers_suppliers.extend(registered_suppliers)
        ...
        Supplier = apps.apps.get_model("suppliers", "Supplier")
        for supplier_email in potential_supplier_email:
            if Supplier.objects.filter(email=supplier_email).first():
                potential.append(Supplier.objects.filter(
                    email=supplier_email).first())
            else:
                potential.append(
                    apps.apps.get_model("core", "CategoryTypeSupplier").objects.filter(
                        primary_email=supplier_email).first())
        return potential


# job notifications end here