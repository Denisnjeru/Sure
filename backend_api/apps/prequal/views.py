...
from apps.prequal.models import (
    FinancialRatio, Prequalification, Category, Section, Question, SupplierResponse, MarkingScheme, CustomLetter, DueDilligenceLetter,
    QualityAssurance, QualityAssuranceQuestion, QualityAssuranceResponse, DueDiligence, DueDiligenceQuestion,
    DueDiligenceSupplier, SupplierCategoryScore, AwardLetter, RegretLetter, CategoryReport, CategoryInvite,
    SupplierPDFResponse,
    ClientDocument, QaTccResponse, QaCr12Response, QaBusinessPermitResponse, QaPinCertificateResponse, QaNcaaResponse,
    QaPoisonsBoardResponse, QaIncorporationCertificateResponse
)
from rest_framework.permissions import IsAuthenticated
from backend.settings.base import BASE_DIR
from . import serializers
from .reports import (
    prequal_bidders_information_report, prequal_evaluation_report_context, 
    QARankingReport, PrequalInterimReport, bidder_locations_report, prequalified_suppliers_report, 
    job_bidder_payments_report, responsive_bidders_report, non_responsive_bidders_report, directors_report, 
    participation_status_report, DueDiligenceRankingReport, job_qed_category_suppliers, download_current_suppliers,
    FinancialRatiosReport
)
from .serializers import (
    JobListSerializer, JobCreateUpdateSerializer, CategoryListSerializer, CategoryCreateUpdateSerializer, PrequalQARatioSerializer,
    SectionListSerializer, SectionCreateUpdateSerializer, QuestionListSerializer, QuestionCreateUpdateSerializer,
    MarkingSchemeSerializer, SRSerializer, QASerializer, QAQSerializer, QARSerializer, SRDocumentSerializer, QuestionArchiveSerializer
)
from .sms import sms_category_notifications
from .tasks import category_questions_upload, EvaluatePrequal, job_questions_upload, ZipQuestionFiles, \
    upload_job_current_suppliers, import_category_suppliers
from .utils import job_criteria_template_download, get_file, send_current_suppliers_letter_email
from ..core.utils import Render, weasy_pdf
from ..core.models import CategoryOrder, SupplierReceipt
from .email_notifications import (
    send_participants_email, email_category_notifications, broadcast_job_notifications, SendEmailNotifications,
    send_job_invite_email, send_job_invite_sms, SendSMSNotifications
)
from ..ocr import ocr
...
class JobView(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        ...
        elif self.action == 'upload_current_suppliers':
            return serializers.JobUpdateCurrentSupplierSerializer
        elif self.action == 'send_current_suppliers_letter':
            return serializers.JobUpdateCurrentSupplierLetterSerializer
        else:
    ...
    def get_queryset(self):
        if self.request.query_params.get('company_id'):
            ...
            return Prequalification.objects.filter(
                company_id=company_id).order_by('id')
        else:
            return Prequalification.objects.filter(
                company_id=self.request.auth.payload['company_id']).order_by('id')
    ...
    def create(self, request, *args, **kwargs):
        ...
        if self.request.query_params.get('company_id'):
            ...
            company_id = request.auth.payload['company_id']
    ...
    def job_details_letters(self, request, job_id):
        ...
        jobs = Prequalification.objects.filter(
            id=job_id,
    ...
    def download_criteria_template(self, request, job_id):
        job = Prequalification.objects.filter(id=job_id).first()
        if job:
            file_path = job_criteria_template_download(job=job)
            context = {
    ...
    def upload_criteria_template(self, request, job_id):
        ...
        if s.is_valid():
            ...
            context = {
                "response_message": "Criteria upload complete",
            }
            return Response(context, status=status.HTTP_200_OK)
        else:
            context = {
    ...
    @action(methods=['post'], detail=False, url_path='upload/current_suppliers/(?P<job_id>\d+)')
    def upload_current_suppliers(self, request, job_id):
        ...
        if s.is_valid():
            s.save()
            context = upload_job_current_suppliers(job_id=job_id)
    ...
    def run_current_suppliers_report(self, request, job_id):
        context = download_current_suppliers(job_id=job_id)
        context = {
    ...
    def run_bidder_locations_report(self, request, job_id):
        # qa_ranking_report = QARankingReport()
        context = bidder_locations_report.delay(job_id=job_id)
        context = {
            "response_message": "Report generation started",
            "task_id": context.task_id
        }
    ...
    @action(methods=['get'], detail=False, url_path='run/prequalified/suppliers/report/(?P<job_id>\d+)')
    def run_prequalified_suppliers_report(self, request, job_id):
        # qa_ranking_report = QARankingReport()
        context = prequalified_suppliers_report.delay(job_id=job_id)
        context = {
            "response_message": "Report generation started",
            "task_id": context.task_id
        }
    ...
    def run_responsive_bidders_report(self, request, job_id):
        context = responsive_bidders_report.delay(job_id=job_id)
        context = {
            "response_message": "Report generation started",
            "task_id": context.task_id
        }
    ...
    def run_non_responsive_bidders_report(self, request, job_id):
        context = non_responsive_bidders_report.delay(job_id=job_id)
        context = {
            "response_message": "Report generation started",
            "task_id": context.task_id
        }
    ...
    def run_participation_status_report(self, request, job_id):
        context = participation_status_report.delay(job_id=job_id)
        context = {
            "response_message": "Report generation started",
            "task_id": context.task_id
        }
    ...
    def run_interim_report(self, request, job_id):
        report_instance = PrequalInterimReport()
    ...
    def upload_category_suppliers(self, request, job_id):
        ...
        instance = Prequalification.objects.filter(
            company_id=company_id, id=job_id
        ).first()
        s = serializers.JobCatSupplierSerializer(data=request.data, instance=instance)
        if s.is_valid():
            s.save()
...
class JobNotificationViewSet(viewsets.ModelViewSet):
    ...
    def broadcast_notifications(self, request, job_id):
        ...
        if s.is_valid():
            result = broadcast_job_notifications.delay(data=request.data)
            context = {
                "task_id": result.task_id
            }
            return Response(context, status=status.HTTP_200_OK)
    ...
    def sms_notifications(self, request, job_id):
        n = SendSMSNotifications()
        result = n.delay(job_id=job_id)
        context = {
    ...
    def email_invite_notifications(self, request, job_id):
        result = send_job_invite_email.delay(job_id=job_id)
        context = {
    ...
    def sms_invite_notifications(self, request, job_id):
        result = send_job_invite_sms.delay(job_id=job_id)
        context = {
            "response_messsage": "SMS invite notification initiated successfully",
            "task_id": result.task_id
        }
...
class CategoryView(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Category.objects.filter(prequalification_id=self.kwargs['job_id']).order_by('id')
    ...
    def create(self, request, *args, **kwargs):
        ...
        self.perform_create(serializer)
        headers = self.get_success_headers(serializer.data)
        category = Category.objects.filter(id=serializer.data['id']).first()
        result = category_questions_upload.delay(category.prequalification_id, serializer.data['id'])
        context = {
            "task_id": result.task_id,
    ...
    def close(self, request, job_id, category_id):
        ...
        if category.is_open == True:
            ...
            category.save()
            if category.send_participant_list_to_supplier:
                send_participants_email.delay(category_id=category_id)
            evaluate = EvaluatePrequal()
            response = evaluate.delay(category_id=category_id)
            context = {
    ...
    def refresh_scores(self, request, job_id, category_id):
        ...
        response = evaluate.delay(category_id=category_id)
        context = {
            "response_message": "Score refresh started",
            "task_id": response.task_id
        }
        return Response(context, status=status.HTTP_200_OK)

    @action(methods=['get'], detail=False, url_path='participants/(?P<category_id>\d+)')
    def participants(self, request, category_id, job_id):
        suppliers = suppliers = apps.apps.get_model('suppliers', 'Supplier').objects.filter(
    ...
    @action(methods=['get'], detail=False, url_path='participants/search/(?P<category_id>\d+)')
    def participant_search(self, request, category_id, job_id):
    ...
    def qualified_bidders(self, request, job_id, category_id):
        category = Category.objects.filter(id=category_id).first()
        pass_score = float(category.pass_score)
        # check if qa exists
        bidders = SupplierCategoryScore.objects.filter(category_id=category_id, score__gte=pass_score)
        s = serializers.SupplierCategoryScoreSerializer(bidders, many=True)
        return Response(s.data, status=status.HTTP_200_OK)

    @action(methods=['get'], detail=False, url_path='unqualified/bidders/(?P<category_id>\d+)')
    def unqualified_bidders(self, request, job_id, category_id):
        category = Category.objects.filter(id=category_id).first()
        pass_score = float(category.pass_score)
        bidders = SupplierCategoryScore.objects.filter(category_id=category_id, score__lt=pass_score)
        s = serializers.SupplierCategoryScoreSerializer(bidders, many=True)
        return Response(s.data, status=status.HTTP_200_OK)

    @action(methods=['post'], detail=False, url_path='')
    def duplicate_category(self, request, job_id, category_id):
        category = Category.objects.filter(id=category_id).first()
        request_data = request.data.copy()
        messages = []
        if request_data["bid_charge"] == "":
            request_data["bid_charge"] = 0
        if request_data["currency"] == "":
            request_data["currency"] = "KES"

        time_now = datetime.datetime.now()
        closing_date = request_data["closing_date"]
        opening_date = request_data["opening_date"]

        if time_now > closing_date or closing_date < opening_date:
            context = {
                "response_message": "The opening date, Closing date and today's date don't compare well.",
                "response": "error"
            }
            return Response(context, status=status.HTTP_200_OK)
        elif closing_date > opening_date < time_now:
            request_data["is_open"] = True
        elif closing_date > opening_date > time_now:
            request_data["is_open"] = False

        s = serializers.CategoryDuplicateSerializer(
            request_data, context={"request": request, "category_id": category_id})
        if s.is_valid():
            s.save()
            return Response(s.data, status=status.HTTP_200_OK)
        else:
    ...
    def run_report(self, request, job_id, category_id, type="xls"):
        ...
        if category is not None:
            ...
            if type == "xls":
                ...
                excel_path = prequal_bidders_information_report(data=data)
                return excel_path
            elif type == "pdf":
                ...
                context = prequal_evaluation_report_context(data)
                context['participant_count'] = len(participants)
                pdf = Render.render(
                    "prequal/reports/technical_evaluation_report.html", context
                )
    ...
    # @action(methods=['post'], detail=False, url_path='send/email/notifications/(?P<category_id>\d+)')
    # def send_email_notifications(self, request, job_id, category_id):
    #     email_category_notifications(data=request.data, files=request.FILES, category_id=category_id)
    #     return Response(status=status.HTTP_200_OK)\

    # @action(methods=['post'], detail=False, url_path='send/sms/notifications/(?P<category_id>\d+)')
    # def send_sms_notifications(self, request, job_id, category_id):
    #     sms_category_notifications(data=request.data, category_id=category_id)
    #     return Response(status=status.HTTP_200_OK)
    ...
    def invited_suppliers(self, request, job_id, category_id):
        invites = CategoryInvite.objects.filter(category_id=category_id)
...
class QuestionView(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Question.objects.filter(section_id=self.kwargs['section_id']).order_by('id')
...
class MarkingSchemeView(viewsets.ModelViewSet):
    def get_queryset(self):
        return MarkingScheme.objects.filter(question_id=self.kwargs['question_id']).order_by('id')
...
class QAView(viewsets.ModelViewSet):
    def get_queryset(self):
        return QualityAssurance.objects.filter(category_id=self.kwargs['category_id']).order_by('id')
    ...
    def get_section_questions(self, request, category_id, section_id):
        ...
        s = serializers.QAQListSerializer(questions, many=True)
        return Response(s.data, status=status.HTTP_200_OK)
    ...
    @action(methods=['post'], detail=False, url_path='submit/response/ratios/(?P<qaq_id>\d+)')
    def submit_ratios_qa_response(self, request, category_id, qaq_id):
        data = request.data.copy()
        user = request.user.id
        data['created_by'] = user
        instance = QualityAssuranceResponse.objects.filter(
        ...
        if instance is not None:
            serializer = PrequalQARatioSerializer(data=data, instance=instance)
        else:
            serializer = PrequalQARatioSerializer(data=data)
    ...
    @action(methods=['get'], detail=False, url_path='participant/ratio/responses/(?P<section_id>\d+)/(?P<participant_id>\d+)')
    def get_participant_ratio_responses(self, request, category_id, section_id, participant_id):
        section = Section.objects.filter(id=section_id).first()
        supplier = apps.apps.get_model("suppliers","Supplier").objects.filter(id=participant_id).first()
        ratio_instance = FinancialRatio.objects.filter(section=section,supplier=supplier).first()
        serializer = serializers.PrequalQARatioSerializer(ratio_instance, many=False)
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class QAQView(viewsets.ModelViewSet):
    def get_queryset(self):
        return QualityAssuranceQuestion.objects.filter(
            quality_assurance_id=self.kwargs['qa_id']).order_by('id')
...
class DDView(viewsets.ModelViewSet):
    def get_queryset(self):
        return DueDiligence.objects.filter(category_id=self.kwargs['category_id']).order_by('id')
...
class AwardLetterView(viewsets.ModelViewSet):
    ...
    # def create_letter(self, request, category_id):
    #     data = request.data.copy()
    #     supplier = apps.apps.get_model('suppliers', 'Supplier').objects.filter(id=data['supplier']).first()
    #     category = Category.objects.filter(id=category_id).first()

    #     if AwardLetter.objects.filter(category_id=category_id, supplier_id=data['supplier']).count() > 0:
    #         context = {
    #             "response_message": "Award letter already exists"
    #         }
    #         return Response(context, status=status.HTTP_400_BAD_REQUEST)
    #     else:
    #         data['name'] = f'{supplier.company_name} {category.name} Award Letter'
    #         data['description'] = f'{supplier.company_name} {category.name} Award Letter'
    #         data['award_date'] = timezone.now()

    #         serializer = self.get_serializer(data=data)
    #         serializer.is_valid(raise_exception=True)
    #         self.perform_create(serializer)
    #         create_award_letter(letter_id=serializer.data['id'])
    #         headers = self.get_success_headers(serializer.data)
    #         return Response(serializer.data, status=status.HTTP_201_CREATED, headers=headers)
    ...
    def multiple_award_letters(self, request, category_id):
        ...
        success_letter = ClientDocument.objects.get(
            prequalification_id=job.id, document_type="success")
        ...
        for supplier_id in supplier_ids:
            ...
            if not regret_letter:
                ...
                template_path = "prequal/letters/success_letter.html"
                context = {
                    ...
                    "company": company,
                    "supplier": supplier,
                    "sourcing_activity": "Prequalification",
                }
                ...
                email_subject = f"{company.company_name} PREQUALIFICATION OUTCOME FOR THE {category.name}".upper()
                message = render_to_string(
                    "prequal/letters/success_email.html",
...
class RegretLetterView(viewsets.ModelViewSet):
    ...
    def multiple_regret_letters(self, request, category_id):
        ...
        regret_letter = get_object_or_404(ClientDocument,
            prequalification_id=job.id, document_type="regret")
        time = datetime.datetime.now()
        ...
        for supplier_id in supplier_ids:
            ...
            if not award_letter:
                ...
                Path(file_path).mkdir(parents=True, exist_ok=True)
                template_path = "prequal/letters/regret_letter.html"
                ...
                email_subject = f"{company.company_name} PREQUALIFICATION OUTCOME FOR THE {category.name}".upper()
                message = render_to_string(
                    "prequal/letters/regret_email.html",
                    {"user": supplier, "category": category,},
                )
                to_email = supplier.username
...
class CustomLetterView(viewsets.ModelViewSet):
    ...
    def multiple_custom_letters(self, request, category_id):
        ...
        custom_letter = get_object_or_404(ClientDocument,
            prequalification_id=job.id, document_type="custom")
        time = datetime.datetime.now()
        ...
        for supplier_id in supplier_ids:
            ...
            Path(file_path).mkdir(parents=True, exist_ok=True)
            template_path = "prequal/letters/custom_letter.html"
            ...
            message = render_to_string(
                "prequal/letters/custom_email.html", {"user": supplier, "category": category},
            )
            to_email = supplier.username
            email = EmailMultiAlternatives(email_subject, message, to=[to_email])
            email.attach_alternative(message, "text/html")
...
class DDLetterView(viewsets.ModelViewSet):
    ...
    def multiple_dd_letters(self, request, category_id):
        ...
        dd_letter = get_object_or_404(ClientDocument,
            prequalification_id=job.id, document_type="dd")
        ...
        for supplier_id in supplier_ids:
            ...
            Path(file_path).mkdir(parents=True, exist_ok=True)
            template_path = "prequal/letters/dd_letter.html"
            context = {
            ...
            email_subject = f"{job.company.company_name} PREQUALIFICATION NOTIFICATION FOR DUE DILLIGENCE FOR {category.name}"
            email_subject = email_subject.upper()
            message = render_to_string(
                "prequal/letters/dd_email.html",
...
class ClientDocumentView(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return ClientDocument.objects.filter(
            prequalification__company_id=company_id).order_by('id')
    ...
    def job_client_document_update(self, request, job_id, type):
        document = ClientDocument.objects.filter(
            prequalification_id=job_id, document_type=type).first()
        if document:
...
class ArchiveView(viewsets.ModelViewSet):
    def get_queryset(self):
        ...
        return Prequalification.objects.filter(company_id=company_id).order_by('id')

    def get_serializer_class(self):
        if self.action == 'get_archive_documents':
            return serializers.SRSerializer
    ...
    def zip_files(self, request, question_id):
        ...
        question = Question.objects.filter(
            id=question_id,
            section__category__prequalification__company_id=company_id).first()
        if question:
            function = ZipQuestionFiles()
            result = function.delay(question_id=question_id)
            context = {
                'task_id': result.task_id,
                'response_message': 'File zipping started'
            }
            return Response(context, status=status.HTTP_200_OK)
    ...
    def get_archive_documents(self, request, category_id, supplier_id):
        ...
        documents = QuestionArchiveSerializer(documents, many=True, context={'supplier_id': self.kwargs['supplier_id']}).data
...
class TestOcrView(viewsets.GenericViewSet):

    def list(self, request):
        # if document is loacl use
        response = ocr.verify_incorporationcert(local_path='/home/gathage/coding/qed/tendersureV2/backend/media/uploads/tcc/certificate_of_incorporation.jpeg')
        # if document is already in s3 use
        # response = ocr.verify_incorporationcert(document='temp/certificate_of_incorporation.jpeg') # NOTE: pass the document key in s3
