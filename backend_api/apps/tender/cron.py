...
def open_close_tender():
    time_utc = datetime.datetime.now(datetime.timezone.utc)

    open_categories = apps.apps.get_model('tender', 'Category').objects.filter(closing_date__gte=time_utc)
    ...
    closing_categories = apps.apps.get_model('tender', 'Category').objects.filter(closing_date__lte=time_utc, is_open=True)
