...
export default {
    ...
    methods: {
        selectSupplier: function(supplier) {
            ...
            this.$router.push('/company/archive/'+ this.$route.params.sourcing_activity +'/categories/' + this.$route.params.categoryId + '/suppliers/' + supplier.id)
        },
        async getSuppliers() {
            try {
                if (this.$route.params.sourcing_activity === 'prequalification') {
                    ...
                    const response = await tender.all_participants(this.$route.params.jobId, this.$route.params.categoryId)
