        ...
        <div class="page__content">
            <div class="columns is-centered">
                <div class="column-details column is-12">
                    <form v-on:submit.prevent="create_foward_auction()">
                        <div class="column-details__head">
                            <p class="column-details__head--title">Create a Foward Auction</p>
                            ...
                            <div class="columns">
                                <div class="column">
                                    <div class="field">
                                        ...
                                        <div class="control">
                                            <input class="input" type="text" placeholder="Enter category name" v-model="foward_auction.name" required>
                                        </div>
                                    </div>
                                    <div class="field">
                                    ...
                                    <div class="field expiry">
                                        ...
                                        <div class="control">
                                            <input class="input" type="datetime-local" placeholder="Enter Opening Date" v-model="foward_auction.opening_date" required>
                                        </div>
                                    </div>
                                    <div class="field expiry">
                                        <label class="label">Closing Time <span class="required">*</span></label>
                                        <div class="control">
                                            <input class="input" type="datetime-local" placeholder="Enter Closing Date" v-model="foward_auction.closing_date" required>
                                        </div>
                                    </div>
                                    <!-- <div class="field">
                                        <label class="label">Pricing Method<span class="required">*</span></label>
                                        <div class="select">
                                            @change="onMethodChange($event)" -- this was commented
                                            <select v-model="selectedPricingMethod" required>
                                                <option>Single note</option>
                                                <option>Multi note</option>
                                            </select>
                                        </div>                                    
                                    </div> -->
                                    <!-- <div class="selected">
                                        <span v-if="selectedPricingMethod !='' ">
                                            <span class="selected__item currency">
                                ...
                                <div class="column">
                                    <div class="field">
                                        <div class="control">
                                            <div class="show_bids">
                                                <input type="checkbox" v-model="foward_auction.closed_auction">
                                        ...
                                        <div class="control">
                                            <input class="input" type="number" min="0" max="10" oninput="this.value = Math.round(this.value);" v-model="foward_auction.overtime_count"/>
                                        </div>
                                    </div>
                                    <div class="field">
                                        <label class="label">Overtime Duration (Minute(s))</label>
                                        <div class="control">
                                            <input class="input" type="number" min="0" max="10" oninput="this.value = Math.round(this.value);" v-model="foward_auction.overtime_duration"/>
                                        </div>
                                    </div>
                                    <div class="document">
        ...
        methods:{
            onTypeChange(e){
                if (e.target.value != 'Select Auction Type'){
                    ...
                    this.foward_auction.auction_type = e.target.value
                }else{
                    this.selectedType = ''
                }
            },
            setTypeTo(){
                this.selectedType = 'Foward Auction'
                this.foward_auction.auction_type = 'Foward Auction'
            },
            onMethodChange(e){
                if (e.target.value != 'Select Pricing Method'){
                    this.selectedPricingMethod = e.target.value
                    this.foward_auction.pricing_method = e.target.value
                }else{
                    this.selectedPricingMethod = ''
                    this.foward_auction.pricing_method = ''
                }
            },
            removeexcelfile(){
                this.foward_auction.excel_template = null
                this.ExcelfileNames = []
            },
            removesupportingdocfile(){
                this.foward_auction.supporting_document = null
            ...
            selectSupportingDocument(event){ 
                ...
                this.foward_auction.supporting_document = event.target.files[0]
                ...
                console.log(this.foward_auction.supporting_document)
            ...
            selectExcelDocument(event){
                ...
                if (!fileList.length) return;            
                this.foward_auction.excel_template = event.target.files[0]
                ...
                console.log(this.foward_auction.excel_template)

            },
            async create_foward_auction(){
                try {
                    if (this.foward_auction.name !== ''  && this.foward_auction.opening_date  !== '' && this.foward_auction.closing_date !== ''){
                        
                        const fd = new FormData()
                        fd.append('name',this.foward_auction.name)
                        fd.append('auction_type',this.foward_auction.auction_type)
                        fd.append('pricing_method',this.foward_auction.pricing_method)
                        fd.append('opening_date',this.foward_auction.opening_date)
                        fd.append('closing_date',this.foward_auction.closing_date)
                        fd.append('auction_type',this.foward_auction.auction_type)
                        fd.append('pricing_method',this.foward_auction.pricing_method)
                        fd.append('closed_auction',this.foward_auction.closed_auction)
                        fd.append('category_type',this.foward_auction.category_type)
                        
                        if (this.foward_auction.supporting_document !== null){
                            fd.append('supporting_document',this.foward_auction.supporting_document)
                        }

                        if (this.foward_auction.excel_template !== null){
                            fd.append('excel_template', this.foward_auction.excel_template)
                        }
                        
                        fd.append('overtime_count',this.foward_auction.overtime_count)
                        fd.append('overtime_duration',this.foward_auction.overtime_duration)

                        const response = await auctions.createFowardAuctions(fd)
                        console.log(response.status)
                        if(response.status == 201){
                            console.log(response.data)
                            window.toast.fire({icon: 'success', title: "Auction created successfully"})
                            this.$router.push('/buyer/'+ response.data.id +'/foward/auction')
                        }
                    }else{
                        // No Name
                        if(this.foward_auction.name == ''){
                            this.error_text = 'Please add auction name !'
                        }
                        // No Opening Date
                        if(this.foward_auction.opening_date == ''){
                            this.error_text = 'Please add opening date !'
                        }
                        // No Closing Date
                        if(this.foward_auction.closing_date == ''){
