    ...
    <div class="dashboard">
      <div class="page__head">
            <span class="page__head--title">
               Prequalification Jobs > {{ prequal.title }}
            </span>
...
<!--            <p class="column-details__head&#45;&#45;desc">Fill in the required details</p>-->
          ...
          <div class="column-details__content">
            ...
            <hr>
            <p v-if="prequal.status === 'draft'"><strong>Approval Status:</strong> Draft <a @click="publish_job()">Click Here To Publish</a></p>
...
<!--            <p v-else><strong>Approval Status:</strong> Closed</p>-->
            <hr>
            <template v-if="prequal.has_participants === false && prequal.is_open === false">
              <div class="field">
                <label class="label">Import Questions From Excel<span class="required"> *</span></label>
                <a @click="generate_criteria_job_template()" class="button is-primary">
                  <span><font-awesome-icon class="view__icon" icon="download"/> Questions Template</span>
                </a>
                <div class="control">
                  <br>
                  <input class="input" type="file" @change="upload_template($event)">
                </div>
              </div>
              <hr>
            </template>
...
<!--            <hr>-->
          ...
          <div class="column-details__content">
            <div class="columns">
              ...
              <div class="column is-8">
                <router-link :to="'/company/prequal/reports/'+prequal.id"
                             class="button is-primary is-block is-small">View Reports</router-link>
              </div>
              <div class="column is-2">
    ...
    <div class="dashboard">
      <div class="page__head">
        ...
        <div class="page__head--links">
          <router-link :to="'/company/prequalification/create/category/' + prequal.id" class="button is-primary is-small">
            Add Category
          </router-link>
        </div>
      </div>
              ...
              <div class="dropdown-menu" id="dropdown-menu3" role="menu">
                <div class="dropdown-content">
                  ...
                  <template v-if="row.is_open === false">
                    ...
                    <router-link :to="'/company/prequalification/edit/category/'+prequal.id + '/' + row.id"
                      class="dropdown-item" style="margin-right: 2px">
                      <p><font-awesome-icon class="view__icon" icon="pencil-alt"/> Edit</p>
                    </router-link>
                    ...
                    <template>
                      ...
                      <router-link :to="'/company/prequal/dd/details/'+row.id" v-else-if="row.has_dd_instance === true"
                                   class="dropdown-item" style="margin-right: 2px">
                    ...
                    <template>
                      ...
                      <router-link :to="'/company/prequal/qa/instructions/'+row.id" v-else-if="row.has_qa_instance === true"
                                   class="dropdown-item" style="margin-right: 2px">
                          <font-awesome-icon class="view__icon" icon="shield-alt"/>
                        QA
                      </router-link>
                  ...
                  <router-link :to="'/company/prequalification/category/details/'+ prequal.id +'/' + row.id"
                           class="dropdown-item">
...
export default {
  ...
  methods: {
    ...
    async publish_job(){
      try{
        let response = await prequal.publish_job(this.prequal.id)
    ...
    async search() {
      if (this.category_search_key != ""){
        try {
          let response = await prequal.category_search(this.$route.params.id, this.category_search_key, this.page, this.dataPerPage)
    ...
    async uploadCategorySuppliers(event){
      ...
      try{
        let response = await prequal.upload_category_suppliers(this.prequal.id, form_data)
        if(response.status === 200){
          window.toast.fire({icon: 'success', title: response.data['response_message']})
          this.$router.push(`/company/prequal/cat/suppliers/progress/${this.prequal.id}/${response.data['task_id']}`)
        }else{
    ...
    async upload_template(event){
      let file = event.target.files[0]
      let form_data = new FormData();
      form_data.append("question_template", file, file.name)

      try{
        let response = await prequal.upload_criteria(form_data, this.prequal.id)
        if (response.status === 200){
          window.toast.fire({icon: 'success', title: response.data['response_message']})
        }else{
          window.toast.fire({icon: 'error', title: 'Error. Try Again!'})
    ...
    async uploadCurrentSuppliers() {
      try {
        ...
        let response = await prequal.upload_current_suppliers(form_data, this.$route.params.id)
        window.toast.fire({icon: 'success', title: response.data.response_message})
      } catch (err) {
    ...
    async sendCurrentSuppliersLetter() {
      try {
        ...
        let response = await prequal.send_current_suppliers_letter(form_data, this.$route.params.id)
        window.toast.fire({icon: 'success', title: response.data.response_message})
      } catch (err) {
        window.toast.fire({icon: 'error', title: err.response.data.response_message})
      }
    ...
    async initiate_qa(row) {
      ...
      try {
        let response = await prequal.initiate_qa(content, row.id)
        console.log(response)
        window.toast.fire({icon: 'success', title: 'Quality assurance initiated successfully'})
        this.$router.push(`/company/prequal/qa/instructions/${row.id}`)
      } catch (err) {
        window.toast.fire({icon: 'error', title: err})
      }
    },
    ...
    async send_invite_email(){
      try{
        let response = await prequal.send_invite_email(this.$route.params.id)
        if (response.status == 200){
          window.toast.fire({icon: 'success', title: response.data["response_message"]})
        }else{
    ...
    async send_invite_sms(){
      try{
        let response = await prequal.send_invite_sms(this.$route.params.id)
        if (response.status == 200){
          window.toast.fire({icon: 'success', title: response.data["response_message"]})
        }else{
          window.toast.fire({icon: 'error', title: "An error occurred! Please Try Again."})
    ...
    async generate_criteria_job_template(){
      try{
        let response = await prequal.criteria_job_template(this.prequal.id)
        if (response.status === 200){
    ...
    async initiate_dd(row) {
      ...
      try {
        let response = await prequal.initiate_dd(content, row.id)
        console.log(response)
        window.toast.fire({icon: 'success', title: 'Due diligence initiated successfully'})
        this.$router.push(`/company/prequal/dd/details/${row.id}`)
      } catch (err) {
        window.toast.fire({icon: 'error', title: err})
      }
    },
    ...
    async send_broadcast_notification(){
      ...
      try{
        let response = await prequal.send_broadcast_notification(this.$route.params.id, this.broadcast_notification)
        if (response.status == 200){
    ...
    async send_email_notification(){
      ...
      try{
        let response = await prequal.send_job_email_notification(this.$route.params.id, formData)
        if (response.status == 200){
    ...
    async send_sms_notification(){
      ...
      try{
        let response = await prequal.send_job_sms_notification(this.$route.params.id, this.sms_notification)
        if (response.status === 200){
          window.toast.fire({icon: 'success', title: response.data['response_message']})
        }else{
          window.toast.fire({icon: 'error', title: 'An error occurred! Please Try Again.'})
