...
<!--            <hr>-->
          ...
          <div class="column-details__content">
            ...
            <div class="columns" v-if="category.invite_only === true">
              ...
              <div class="column is-8">
                <router-link
                    :to="'/company/prequal/invited/suppliers/'+$route.params.prequal_id+'/'+$route.params.category_id"
          ...
          <div class="page__head">
            ...
            <div class="page__head--links column is-8" style="display: inline !important;">
              <div class="columns">
                ...
                <div class="column is-6">
                  <router-link :to="'/company/prequal/create/section/' + prequal.id + '/' + category.id"
            ...
            <div class="table-search">
              ...
              <v-client-table :columns="section_columns" :options="section_options" :data="sections">
                ...
                <span slot="Actions" slot-scope="{row}" class="is-justify-content-right">
                  ...
                  <router-link :to="'/company/prequal/edit/section/'+prequal.id+'/'+category.id+'/'+row.id"
                               class="button is-primary is-small" style="margin-right: 2px;">
                      <p><font-awesome-icon class="view__icon" icon="pencil-alt"/></p>
                  </router-link>

                  <router-link :to="'/company/prequal/section/questions/' + row.id"
              ...
              <span class="page__head--title column is-4">
                ...
                <div class="page__head--links column is-8" style="display: inline !important;">
                    ...
                    <template v-else>
                      <div class="columns">
                        <div class="column is-6">
<!--                          <router-link :to="'/company/prequal/letters/'+prequal.id+'/'+category.id" class="button is-primary is-fullwidth" style="margin-right: 2px">-->
<!--                            Letters-->
<!--                          </router-link>-->
                        </div>
          ...
          <div class="page__content columns bottom_content">
            <div class="table-search">
                ...
                <div class="table-search__search">
                  <font-awesome-icon class="table-search__search--icon" icon="search"/>
                  <input @keyup.enter="searchParticipants()" class="table-search__search--input" type="text" id="participant_query"
                         placeholder="Search here">
                </div>
              </div>
...
<!--                  </span>-->
                  ...
                  <template v-if="category.is_open === false && category.has_qa_instance === true">
                     <router-link :to="'/company/prequal/conduct/qa/'+ category.id +'/' + row.id"
                               class="button is-primary is-small">
...
export default {
  ...
  methods: {
    ...
    async section_search() {
      ...
      try{
        let response = await prequal.section_search(this.$route.params.category_id, query, this.section_data_per_page)
        if (response.status === 200){
    ...
    async getSections(){
      try{
        let response = await prequal.sections(this.$route.params.category_id, this.section_page, this.section_data_per_page)
        if (response.status === 200){
          this.sections = response.data['results']
          this.section_data_count = response.data['count']
        }else{
    ...
    async searchParticipants(){
      try{
        let query = document.getElementById('participant_query').value
        let response = await prequal.search_category_participants(
          this.$route.params.category_id, this.$route.params.prequal_id, this.page, query)
