...
export default {
  ...
  methods: {
    async update() {
      ...
      form_data.append("prequalification", parseInt(this.form.prequalification));

      try {
        let response = await prequal.update_category(form_data, this.$route.params.prequal_id, this.$route.params.category_id)
        window.toast.fire({icon: 'success', title: "Prequalification job created successfully"})
        this.$router.push(`/company/prequalification/category/details/${this.form.prequalification}/${response.data['id']}`)
      } catch (err) {
    ...
    async get_category(){
      try{
        ...
        this.form.prequalification = response.data['prequalification'].id
