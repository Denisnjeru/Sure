...
<div class="dashboard">
    <div class="page__head">
      <span class="page__head--title">
         Letters > Prequalification Job > Success
      </span>
...
export default {
  ...
  methods:{
    ...
    async getLetter(){
      try{
        let response = await prequal.job_client_document(this.$route.params.job_id, 'success')
    ...
    async submitLetter(){
      ...
      form_data.append("document_type", "success")
      form_data.append("tendersure_module", "letters")

      try{
        let response = await prequal.update_client_document(form_data, this.$route.params.job_id, 'success')
        if(response.status === 200){
          this.$router.push(`/company/prequal/letters/${this.$route.params.job_id}`)
        }else{
          window.toast.fire({icon: 'error', title: 'Error. Please Try Again!'})
        }
      }catch(err){
