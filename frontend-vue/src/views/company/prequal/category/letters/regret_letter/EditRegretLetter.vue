...
<div class="dashboard">
    <div class="page__head">
      <span class="page__head--title">
         Letters > Prequalification Job > Regret
      </span>
...
export default {
  ...
  methods:{
    ...
    async getLetter(){
      try{
        let response = await prequal.job_client_document(this.$route.params.job_id, 'regret')
    ...
    async submitLetter(){
      ...
      try{
        let response = await prequal.update_client_document(form_data, this.$route.params.job_id, 'regret')
        if(response.status === 200){
