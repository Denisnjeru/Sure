...
export default {
  ...
  methods: {
    async create(){
      try{
        await prequal.update_section(this.$route.params.category_id, this.$route.params.section_id, this.form)
        window.toast.fire({icon: 'success', title: "Section updated successfully"})
        this.$router.push(`/company/prequalification/category/details/${this.$route.params.prequal_id}/${this.$route.params.category_id}`)
      }catch (err){
    ...
    async get_section(){
      try {
        let response = await prequal.section(this.$route.params.category_id, this.$route.params.section_id)
