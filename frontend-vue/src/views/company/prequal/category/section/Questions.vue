  ...
  <div class="dashboard">
        <div class="page__head">
            ...
            <div class="page__head--links">
                    <router-link :to="'/company/prequal/create/question/'+$route.params.section_id" class="button is-primary is-small">
                        New Question
                    </router-link>
            </div>
        </div>
            ...
            <div class="column is-12 column-page">
                <div class="table-search">
                    ...
                    <div class="table-search__search">
                        ...
                        <input @keyup.enter="search()" class="table-search__search--input" type="text" placeholder="Search here" id="query">
                    </div>
                </div>
              ...
              <v-client-table :columns="columns" :options="options" :data="questions">
                ...
                <span slot="Actions" slot-scope="{row}">
                  <router-link :to="'/company/prequal/edit/question/'+$route.params.section_id+'/'+row.id" class="button is-primary is-small">
...
export default {
  ...
  methods: {
    async zipFiles(question_id){
      try{
        let response = await prequal.zip_files(question_id)
        window.toast.fire({icon: 'success', title: response.data['response_message']})
        this.$router.push(`/company/prequal/zip/files/progress/`+response.data['task_id']+'/'+this.$route.params.section_id)
      }catch (err){
    ...
    async search() {
      try{
        ...
        let response = await prequal.search_questions(this.$route.params.section_id, key, this.page, this.dataPerPage)
    ...
    // async fetchData() {
    //     try{
    //       let response = await prequal.questions_filter(this.$route.params.section_id, this.page, this.dataPerPage)
    //       this.questions = response.data['results']
    //       this.dataCount = response.data['count']
    //       // this.dataPerPage = response.data['count']
    //     }catch (err){
    ...
    async get_questions(){
      try{
        let response = await prequal.questions(this.$route.params.section_id, this.page, this.dataPerPage)
        this.questions = response.data['results']
        this.dataCount = response.data['count']
        // this.dataPerPage = response.data['count']
      }catch (err){
