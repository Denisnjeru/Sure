        ...
        <div class="page__content">
            <div class="columns is-centered">
                <div class="column-details column is-4">
                    <form v-on:submit.prevent="create_risk_job()">
                        <div class="column-details__head">
                            <p class="column-details__head--title">Add New Risk Management Job</p>
                        ...
                        <div class="column-details__content">
                            <div class="field">
                                ...
                                <div class="control">
                                    <input class="input" type="text" placeholder="Enter Job Title" v-model="risk.title">
                                </div>
                            </div>
                            <div class="field">
                                <label class="label">Job Code<span class="required">*</span></label>
                                <div class="control">
                                    <input class="input" type="text" placeholder="Enter Job Code" v-model="risk.unique_reference">
                                </div>
                            </div>
                            <div class="field">
                                <div class="control">
                                    <div class="show_bids">
                                        <input type="checkbox" v-model="risk.send_participant_list_to_supplier">
...
export default {
    name:'add-risk-management-job-buyer',
    data() {
        return{
            risk: {
                "title": '',
    ...
    methods:{
        async create_risk_job(){
            try {
                if (this.risk.title !== '' && this.risk.unique_reference !== ''){
                    const response = await risk_management.createRisks(this.risk)
                    
                    if(response.status == 201){
                        console.log(response.data)
                        window.toast.fire({icon: 'success', title: "Risk Management job created successfully"})
                        this.$router.push(`/buyer/details/job/${response.data['id']}/risk-management/`)
                    }else{
                        window.toast.fire({icon: 'error', title: "Error creating risk management job"})
                    } 
                } else {
                    // No Title
                    if(this.risk.title == ''){
                        this.error_text = 'Please add job title !'
                    }
                    // No Unique Reference
