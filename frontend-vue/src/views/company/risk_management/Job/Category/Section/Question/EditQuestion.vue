        ...
        <div class="page__content">
            <div class="columns is-centered">
                <div class="column-details column is-6">
                    <form v-on:submit.prevent="update_question()">
                        <div class="column-details__head">
                            <p class="column-details__head--title">Edit Question </p>
                            <p class="column-details__head--desc">Update in the required details.</p>
                        </div>
                        <!-- Form Details -->
...
import risk_management from '@/services/company/risk_management'
    ...
    export default {
        name:'edit-risk-management-question-buyer',
        data(){
            return{
                "question":{
                    'description': '',
        ...
        methods:{
            async getRiskQuestion(){
                try {
                    const response = await risk_management.getRiskQuestion(this.$route.params.sectionId, this.$route.params.questionId)
            ...
            async update_question(){
                try {
                    if (this.question.description !== ''){
                        const response = await risk_management.updateRiskQuestion(this.question, this.$route.params.sectionId, this.$route.params.questionId)
                        
                        if(response.status == 201){
                            console.log(response.data)
                            window.toast.fire({icon: 'success', title: "Question updated successfully"})
                            this.$router.push('/buyer/edit/question/'+ this.$route.params.sectionId +'/'+ response.data['id'] +'/risk-management/')
                        }else{
                            window.toast.fire({icon: 'error', title: "Error updating question"})
                        } 
                    } else {
                        // No Name
                        if(this.question.description == ''){
