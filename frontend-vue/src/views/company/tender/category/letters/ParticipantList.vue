...
export default {
  ...
  methods: {
    ...
    async get_qualified_bidders(){
      try{
        let response = await tender.qualified_bidders(
          this.$route.params.job_id, this.$route.params.category_id, this.qualified_page, this.qualifiedDataPerPage)
        this.qualified_bidders = response.data['results']
        this.qualifiedDataCount = response.data['count']
      }catch (err){
    ...
    async get_unqualified_bidders(){
      try{
        let response = await tender.unqualified_bidders(
          this.$route.params.job_id, this.$route.params.category_id, this.unqualified_page, this.unqualifiedDataPerPage)
        this.unqualified_bidders = response.data['results']
        this.unqualifiedDataCount = response.data['count']
      }catch (err){
        window.toast.fire({icon: 'error', title: err})
      }
    },
