...
export default {
  ...
  methods:{
    ...
    async getLetter(){
      try{
        let response = await tender.job_client_document(this.$route.params.job_id, 'dd')
    ...
    async submitLetter(){
      ...
      try{
        let response = await tender.update_client_document(form_data, this.$route.params.job_id, 'dd')
        if(response.status === 200){
