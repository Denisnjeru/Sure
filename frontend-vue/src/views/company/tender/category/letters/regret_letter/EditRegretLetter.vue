...
export default {
  ...
  methods:{
    ...
    async getLetter(){
      try{
        let response = await tender.job_client_document(this.$route.params.job_id, 'regret')
    ...
    async submitLetter(){
      ...
      form_data.append("document_type", "regret")
      form_data.append("tendersure_module", "letters")

      try{
        let response = await tender.update_client_document(form_data, this.$route.params.job_id, 'regret')
        if(response.status === 200){
