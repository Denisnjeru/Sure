...
<!--                                  frameBorder="0" scrolling="auto" height="100%" width="100%" ></iframe>-->
              ...
              <template v-else>
                <div class="field">
                  <router-link :to="'/company/tender/category/details/'+sections[0].prequal_id+'/'+$route.params.category_id" type="button" class="button is-primary is-small is-pulled-right">Finish</router-link>
                </div>
              </template>
            </div>
          </div>
  ...
  methods: {
    ...
    async get_questions(section) {
      ...
      try {
        ...
        if(this.active_section.name === "Financial Ratios"){
          this.section_questions = response.data['qa_questions']
          this.section_questions.shift()
        }else{
          this.section_questions = response.data['qa_questions']
        }
    ...
    async submit_ratios_qa(){
      ...
      try{
        let response = await tender.ratios_qa(category_id, supplier_id, this.ratios_form)
