...
<!--              </a>-->
              <hr class="dropdown-divider">
              <a href="#" @click="generate_savings_report()" class="dropdown-item">
                <font-awesome-icon class="view__icon" icon="receipt"/>
                Savings Report
              </a>
              <hr class="dropdown-divider">
              <a href="#" class="dropdown-item" @click="generate_qa_report()">
                <font-awesome-icon class="view__icon" icon="receipt"/>
                QA Ranking Report
              </a>
              <hr class="dropdown-divider">
            ...
            <div class="dropdown" :id="'row_'+row.row.id">
              ...
              <div class="dropdown-menu" id="dropdown-menu3" role="menu">
                <div class="dropdown-content">
                  ...
                  <hr class="dropdown-divider">
                  <a class="dropdown-item" @click="generate_financial_report(row.row.id)">
                    <font-awesome-icon class="view__icon" icon="receipt"/>
...
export default {
    ...
    return {
      ...
      options: {
        ...
        async requestFunction(data) {
          console.log(data)
          try {
            let response = await tender.report_categories(this.$route.params.id)
            let response_data = {
  ...
  methods: {
    ...
    async generate_qa_report(){
      try{
        ...
        if(response.status === 200){
          ...
          this.$router.push(`/company/tender/report/progress/${this.$route.params.id}/${response.data['task_id']}`)
        }else{
    ...
    async generate_savings_report(){
      try{
        let response = await tender.savings_report(this.$route.params.id)
        if(response.status === 200){
          window.toast.fire({icon: 'success', title: "Report generation started"})
          this.$router.push(`/company/tender/report/progress/${this.$route.params.id}/${response.data['task_id']}`)
        }else{
    ...
    async generate_financial_report(category_id){
      try{
        let response = await tender.financial_report(this.$route.params.id, category_id)
        if(response.status === 200){
          window.toast.fire({icon: 'success', title: "Report generation started"})
          this.$router.push(`/company/tender/report/progress/${this.$route.params.id}/${response.data['task_id']}`)
        }else{
    ...
    async get_ratios_report(category_id){
      try{
        let response = await tender.ratios_report(category_id)
        if(response.status === 200){
          window.toast.fire({icon: 'success', title: "Report generation started"})
          this.$router.push(`/company/tender/report/progress/${this.$route.params.id}/${response.data['task_id']}`)
        }else{
