...
<script>
import archive from '@/services/qed/archive'

export default {
    name: 'BWQedArchiveCompanies',
    data() {
        return {
    ...
    methods: {
        selectBuyer: function(buyer) {
            this.$store.dispatch('Archive/setSelectedBuyer', {buyer})
            this.$router.push('/qed/archive/buyers/' + buyer.id + '/jobs')
        },
        async getBuyers() {
            try {
                const response = await archive.buyers(this.page, this.dataPerPage)
