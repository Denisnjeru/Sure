<template>
    <div class="dashboard">
        ...
        <form class="form" v-on:submit.prevent="createContract()" enctype="multipart/form-data">
        <div class="page__content columns">
            <div class="column is-6 column-details">
                <div class="column-details__head">
                    <p class="column-details__head--title">Buyer Details</p>
                    <p class="column-details__head--desc"></p>
                </div>
                <div class="column-details__content">
                    <div class="field">
                        <label class="label">Job <span class="required">*</span></label>
                        <div class="control">
                            <input readonly class="input readonly" type="text" :value="selectedJob.title">
                        </div>
                    </div> 
                    <div class="field">
                        <label class="label">Buyer <span class="required">*</span></label>
                        <div class="control">
                            <input readonly class="input readonly" type="text" :value="selectedBuyer.company_name">
                        </div>
                    </div>  
                    <div class="field">
                        <label class="label">Buyer Initials <span class="required">*</span></label>
                        <div class="control">
                            <input readonly class="input readonly" type="text" :value="selectedBuyer.buyer_initials">
                        </div>
                    </div> 
                    <div class="field">
                        <label class="label">Contact Phone Number <span class="required">*</span></label>
                        <div class="control">
                            <input readonly class="input readonly" type="text" :value="selectedBuyer.phone_number">
                        </div>
                    </div> 
            ...
            <div class="column is-6 column-details">
                ...
                <div class="column-details__content">
                    <div class="field">
                        ...
                        <div class="control">
                            <input v-model="contract.start_date" required class="input" type="date">
                        </div>
                    </div>
                    <div class="field">
                        <label class="label">Contract End Date <span class="required">*</span></label>
                        <div class="control">
                            <input v-model="contract.end_date" required class="input" type="date">
                        </div>
                    </div>
                    <div class="field">
                ...
                <div class="page__content columns">
                    <div class="column is-9 column-page">
                        ...
                        <v-client-table :columns="templateColumns" :data="contractTemplates" :options="options" class="hasNoWrap">
                            <span class="view" slot="template" slot-scope="{row}">
...
export default {
    ...
    computed: {
        ...mapGetters('Contracts',['selectedJob', 'selectedBuyer']),
    },
    mounted() {
    ...
    methods: {
        ...
        async createContract() {
            try {             
                ...
                fd.append('contract_with','company')
                fd.append('entity_id',this.selectedBuyer.id)
                fd.append('contract_for','job')
                fd.append('target_id',this.selectedJob.id)
                ...
                fd.append('content','')
                fd.append('source',this.contractSource)
                ...
                if (this.contractSource === 'sections') {
                    fd.append('source_list', this.selectedContractSections.map(item => item['id']));
                } else if (this.contractSource === 'templates') {
                    fd.append('source_list', this.selectedContractTemplate.map(item => item['id']));
                }

                const response = await contracts.createContract(fd)
                window.toast.fire({
                    icon: 'success',
                    title: 'Contract created successfully'
                })

                this.$router.push('/qed/contracts/buyer/contract/' + response.data.id + '/update')
            } catch (err) {
                console.log(err)
            }
        },
