...
<!--            <hr>-->
          ...
          <div class="column-details__content">
            <div class="columns">
              ...
              <div class="column is-8">
                <router-link :to="'/qed/prequal/reports/'+prequal.id"
    ...
    <div class="dashboard">
      <div class="page__head">
        ...
        <div class="page__head--links">
          <router-link :to="'/qed/prequalification/create/category/' + prequal.id" class="button is-primary is-small">
            Add Category
          </router-link>
        </div>
      </div>
              ...
              <div class="dropdown-menu" id="dropdown-menu3" role="menu">
                <div class="dropdown-content">
                  ...
                  <template v-if="row.is_open === false">
                    ...
                    <router-link :to="'/qed/prequalification/edit/category/'+prequal.id + '/' + row.id"
                    ...
                    <template>
                      ...
                      <router-link :to="'/qed/prequal/dd/details/'+row.id" v-else-if="row.has_dd_instance === true"
                                   class="dropdown-item" style="margin-right: 2px">
                    ...
                    <template>
                      ...
                      <router-link :to="'/qed/prequal/qa/instructions/'+row.id" v-else-if="row.has_qa_instance === true"
                                   class="dropdown-item" style="margin-right: 2px">
                          <font-awesome-icon class="view__icon" icon="shield-alt"/>
                        QA
                      </router-link>
                  ...
                  <router-link :to="'/qed/prequalification/category/details/'+ prequal.id +'/' + row.id"
                           class="dropdown-item">
  ...
  methods: {
    ...
    async uploadCategorySuppliers(event){
      ...
      try{
        ...
        if(response.status === 200){
          window.toast.fire({icon: 'success', title: response.data['response_message']})
          this.$router.push(`/qed/prequal/cat/suppliers/progress/${this.prequal.id}/${response.data['task_id']}`)
        }else{
    ...
    async getPrequal() {
      try {
        let response = await prequal.prequal(this.$route.params.id, this.selectedBuyer.id)
    ...
    async getCategories() {
      try {
        let response = await prequal.categories(this.$route.params.id, this.page, this.selectedBuyer.id)
    ...
    async initiate_qa(row) {
      ...
      try {
        ...
        this.$router.push(`/qed/prequal/qa/instructions/${row.id}`)
      } catch (err) {
        window.toast.fire({icon: 'error', title: err})
      }
    },
    ...
    async initiate_dd(row) {
      ...
      try {
        ...
        this.$router.push(`/qed/prequal/dd/details/${row.id}`)
      } catch (err) {
        window.toast.fire({icon: 'error', title: err})
      }
    },
