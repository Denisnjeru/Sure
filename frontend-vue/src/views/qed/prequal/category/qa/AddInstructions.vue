        ...
        <div class="columns" v-if="section_questions">
          <div class="column is-12" style="padding: 16px 24px;">
            <div id="accordion_second">
              ...
              <template v-else>
                <div class="field">
                  <router-link :to="'/qed/prequalification/category/details/'+sections[0].prequal_id+'/'+$route.params.category_id" type="button" class="button is-primary is-small is-pulled-right">Finish</router-link>
                </div>
              </template>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    async get_questions(section) {
      ...
      try {
        let response = await prequal.qa_section_questions(this.$route.params.category_id, section.id)
    ...
    async submit_instructions(question_id){
      ...
      try {
        await prequal.submit_instructions(content, this.$route.params.category_id, question_id)
