  ...
  <div class="dashboard">
        <div class="page__head">
            ...
            <div class="page__head--links">
                    <router-link :to="'/qed/prequal/create/question/'+$route.params.section_id" class="button is-primary">
                        New Question
                    </router-link>
            </div>
        </div>
            ...
            <div class="column is-12 column-page">
              ...
              <v-client-table :columns="columns" :options="options" :data="questions">
                ...
                <span slot="Actions" slot-scope="{row}">
                  <router-link :to="'/qed/prequal/edit/question/'+$route.params.section_id+'/'+row.id" class="button is-primary is-small">
  ...
  methods: {
    async zipFiles(question_id){
      try{
        ...
        this.$router.push(`/qed/prequal/zip/files/progress/`+response.data['task_id']+'/'+this.$route.params.section_id)
      }catch (err){
    ...
    async fetchData() {
        try{
          let response = await prequal.questions_filter(this.$route.params.section_id, this.page)
    ...
    async get_questions(){
      try{
        let response = await prequal.questions(this.$route.params.section_id)
        this.questions = response.data['results']
        this.dataCount = response.data['count']
        // this.dataPerPage = response.data['count']
      }catch (err){
