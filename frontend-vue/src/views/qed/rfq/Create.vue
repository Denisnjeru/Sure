        ...
        <div class="column-details column is-4">
          <form v-on:submit.prevent="create()">
            <div class="column-details__head">
              <p class="column-details__head--title">Add New Job to Demo Company</p>
...
export default {
  ...
  methods:{
    async create(){
      ...
      try{
        let response = await rfq.create(form_data, this.selectedBuyer.id)
        console.log(response.data)
        window.toast.fire({icon: 'success', title: "RFQ job created successfully"})
        this.$router.push(`/qed/rfq/details/${response.data['id']}`)
      }catch (err){
