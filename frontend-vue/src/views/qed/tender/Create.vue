        ...
        <div class="column-details column is-4">
          <form v-on:submit.prevent="create()">
            <div class="column-details__head">
              <p class="column-details__head--title">Add New Job to {{selecte}}</p>
            ...
            <div class="column-details__content">
              ...
              <div class="field">
                <label class="label">Tender Type<span class="required">*</span></label>
                <div class="select is-fullwidth">
                  <select v-model="form.type">
                    <option selected disabled>Select Tender Type</option>
                    <option value="1">Multi-Item Tender</option>
                    <option value="2">Consolidated Tender</option>
                  </select>
                </div>
              </div>

              <div class="field">
                <label class="label">Advert<span class="requcompanyired"></span></label>
                <div class="control">
...
export default {
  ...
  methods: {
    async create(){
      ...
      try{
        let response = await tender.create(form_data, this.selectedBuyer.id)
        window.toast.fire({icon: 'success', title: "Tender jcompanyob created successfully"})
        this.$router.push(`/qed/tender/details/${response.data['id']}`)
      }catch (err){
    ...
    async get_criteria_countries(){
      try{
        let response = await tender.criteria_countries()
