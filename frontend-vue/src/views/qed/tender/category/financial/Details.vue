    ...
    <div class="dashboard">
      <div class="page__head">
        ...
        <div class="page__head--links">
          <router-link class="button is-primary" :to="'/qed/tender/category/details/'+tender.id+'/'+category.id">
            Technical Details
          </router-link>
        </div>
      </div>
          ...
          <div class="column-details__content">
            ...
            <template v-if="category.has_participants === false && category.is_open === false">
                <div class="field">
                  ...
                  <a download="download" :href="category.items_template" class="button is-primary">
...
<!--                </span>-->
            ...
            <div class="page__pagination">
              <pagination :records="itemDataCount" v-model="page" :per-page="dataPerPage" @paginate="fetchData()">
              </pagination>
            </div>
          </div>
              ...
              <span class="page__head--title column is-4">
                ...
                <div class="page__head--links column is-8" style="display: inline !important;">
                    ...
                    <template v-else>
                      <div class="columns">
                        <div class="column is-6">
<!--                          <router-link :to="'/qed/tender/letters/'+tender.id+'/'+category.id" class="button is-primary is-fullwidth" style="margin-right: 2px">-->
<!--                            Letters-->
<!--                          </router-link>-->
                        </div>
...
<!--                     <router-link :to="'/qed/tender/conduct/qa/'+ category.id +'/' + row.id"-->
<!--                               class="button is-primary is-small">-->
...
<!--                  </template>-->
            ...
            <div class="page__pagination">
              <pagination :records="participantDataCount" v-model="page" :per-page="dataPerPage" @paginate="fetchData()">
              </pagination>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async getParticipants() {
      try {
        let response = await tender.financial_participants(this.$route.params.tender_id, this.$route.params.category_id)
        this.participants = response.data['data']
        this.participantDataCount = response.data['recordsTotal']
      } catch (err) {
        window.toast.fire({icon: 'error', title: err})
      }
    },
    async getItems() {
      try {
        let response = await tender.items(this.$route.params.tender_id, this.$route.params.category_id)
        this.items = response.data['data']
        this.itemDataCount = response.data['recordsTotal']
      } catch (err) {
        window.toast.fire({icon: 'error', title: err})
      }
    },
