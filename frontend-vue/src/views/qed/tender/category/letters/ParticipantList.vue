                    ...
                    <div class="dropdown-menu" id="dropdown-menu3" role="menu">
                      <template v-if="row.has_award_letter === true">
                        <div class="dropdown-content">
                          <router-link :to="`/qed/tender/preview/award/letter/${$route.params.job_id}/${$route.params.category_id}/${row.supplier.id}`" class="dropdown-item">
                      ...
                      <template v-else-if="row.has_regret_letter === true">
                        <div class="dropdown-content">
                          <router-link :to="`/qed/tender/preview/regret/letter/${$route.params.job_id}/${$route.params.category_id}/${row.supplier.id}`" class="dropdown-item">
            ...
            <div class="column is-12 column-page">
              ...
              <v-client-table :columns="columns" :options="options" :data="unqualified_bidders">
                ...
                <span slot="Actions" slot-scope="{row}">
                  <div class="dropdown" :id="'row_'+row.id">
                    <div class="dropdown-trigger">
                      <button class="button is-primary is-small" @click="show_options(row)" aria-haspopup="true" aria-controls="dropdown-menu4">
                        <span>Letter Options</span>
                        <span class="icon is-small">
                          <i class="angle-down" aria-hidden="true"></i>
                        </span>
...
export default {
  ...
  data () {
    return {
      ...
      columns: ['Select', 'Company Name', 'Score', 'Rank', 'Actions'],
      options: {
  ...
  methods: {
    ...
    async create_award_letter(row){
      ...
      try{
        ...
        this.$router.push(
            `/qed/tender/preview/award/letter/${this.$route.params.job_id}/${this.$route.params.category_id}/${row.supplier.id}`
        )
      }catch (err){
        window.toast.fire({icon: 'error', title: err})
      }
    ...
    async create_regret_letter(row){
      ...
      try{
        ...
        this.$router.push(
            `/qed/tender/preview/regret/letter/${this.$route.params.job_id}/${this.$route.params.category_id}/${row.supplier.id}`
        )
      }catch (err){
        window.toast.fire({icon: 'error', title: err})
      }
    ...
    async create_multiple_award_letter(){
      ...
      try{
        let response = await tender.create_multiple_award_letter(
    ...
    async create_mutiple_regret_letter(){
      ...
      try{
        let response = await tender.create_multiple_regret_letter(content, this.$route.params.job_id, this.$route.params.category_id)
        if (response.status === 200){
          this.supplier_ids = []
