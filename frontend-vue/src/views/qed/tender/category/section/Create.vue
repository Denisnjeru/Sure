...
export default {
  ...
  methods: {
    async create(){
      try{
        await tender.create_section(this.$route.params.category_id, this.form)
        window.toast.fire({icon: 'success', title: "Section created successfully"})
        this.$router.push(`/qed/tender/category/details/${this.$route.params.tender_id}/${this.$route.params.category_id}`)
      }catch (err){
