...
export default {
  ...
  methods: {
    ...
    async update() {
      ...
      try {
        await tender.update_question(this.form, this.$route.params.section_id, this.question.id)
        window.toast.fire({icon: 'success', title: "Question created successfully"})
        this.$router.push(`/qed/tender/section/questions/${this.$route.params.section_id}`)
      } catch (err) {
    ...
    async get_question(){
      try {
        let response = await tender.question(this.$route.params.section_id, this.$route.params.id)
