...
export default {
    name: 'foward-auction-list-supplier',
    data () {
        return{
            columns: ['Auction Title', 'Company', 'Items', 'Opening Date', 'Closing Date', 'Status', 'Actions'],
            foward_auctions: [],
            options: {
                headings : {
                ...
                sortable:['Title', 'Company', 'Items', 'Opening Date', 'Closing Date', 'Status'],
                sortIcon: {
                    is: "glyphicon-sort",
                    base: "glyphicon",
    ...
    methods:{
        async onRowClick(act) {
            this.$router.push('/supplier/'+ act.id +'/foward/auction')
        },
        async getFowardAuctions(){
            try{
                const response = await auctions.supplierFowardAuctions()
                console.log(response.data)
                this.foward_auctions = response.data.results
            }catch(error){
                console.log(error)
            }
        },
