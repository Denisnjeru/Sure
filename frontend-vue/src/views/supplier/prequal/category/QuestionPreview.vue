...
<div class="dashboard">
    <div class="page__head">
      <span class="page__head--title">
         Prequalification > Category > Question Preview
      </span>
...
<!--                  <input @keyup.enter="search()" class="table-search__search&#45;&#45;input" type="text" placeholder="Search here">-->
            ...
            <div v-for="section in sections" v-bind:key="section.id">
              <template v-if="section.name !== 'Financial Ratios'">
                ...
                <div :id="'collapsible-message-'+section.id" class="message-body is-collapsible" data-parent="accordion_second" data-allow-multiple="true">
                        ...
                        <div class="column content is-12">
                          <div class="field" v-for="question in section.questions" v-bind:key="question.id">
                            ...
                            <template v-if="question.answer_type === 1">
                              <ul>
                                <li v-if="question.response.options !== null">Response: {{ question.response.options }}</li>
                            ...
                            <template v-else-if="question.answer_type === 5">
                              <ul>
                                <li v-if="question.response.document_url !== null">Response: {{ question.response.document_url }}</li>
                                <li v-else >Response: No response submitted</li>
                              </ul>
                            </template>
...
export default {
  ...
  methods:{
    async getSections(){
      try{
        let response = await supplier_prequal.preview_sections(this.$route.params.category_id)
    ...
    async getParticipationProgress(){
      try{
        let response = await supplier_prequal.participation_progress(this.$route.params.category_id)
        if (response.status === 200){
