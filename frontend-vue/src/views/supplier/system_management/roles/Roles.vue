    ...
    <div class="dashboard">
        <div class="page__head">
            ...
            <div class="page__head--links">
                <router-link :to="'/supplier/user/roles/create'">
            ...
            <div class="column is-12 column-page">
                ...
                <v-client-table :columns="columns" :data="roles" :options="options" class="hasRowNo">
                    ...
                    <span class="view" slot="role" slot-scope="{row}">
                        <router-link :to="'/supplier/user/roles/' + row.id + '/update'">
                            <span>{{row.role}}</span>
                        </router-link>
                    </span>
                    ...
                    <span class="actions" slot="actions" slot-scope="{row}">
                        ...
                        <router-link :to="'/supplier/user/roles/' + row.id + '/privileges'">
                            <span class="actions__edit">
                               Privileges
                            </span>
                        </router-link>
...
export default {
    ...
    methods: {
        ...
        async deleteRole(role) {           
            try {
                await system_management.deleteRole(role.id)                
            } catch (err) {
                window.toast.fire({
                    icon: 'success',
