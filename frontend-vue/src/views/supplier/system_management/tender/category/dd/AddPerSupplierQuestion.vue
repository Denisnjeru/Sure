...
export default {
  ...
  methods: {
    async get_participant(){
      try{
        let response = await tender.dd_participant(
    ...
    async create() {
      ...
      try {
        await tender.create_dd_supplier_question(
        ...
        this.$router.push(
            `/company/tender/dd/supplier/questions/${this.$route.params.category_id}/${this.$route.params.participant_id}`
        )
      } catch (err) {
        window.toast.fire({icon: 'error', title: err})
      }
