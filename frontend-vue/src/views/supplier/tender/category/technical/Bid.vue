...
<div class="dashboard">
    <div class="page__head">
            <span class="page__head--title">
               Tender > Category > Technical Envelope
            </span>

      <div class="page__head--links">
        <router-link :to="'/supplier/tender/preview/'+$route.params.category_id" class="button is-primary is-small">
          Preview
        </router-link>
                  ...
                  <a :href="'#collapsible-message-'+section_questions[0].id">
                    ...
                    <div class="message-body is-collapsible" data-parent="accordion_second" data-allow-multiple="true">
                      <div class="message-body-content">
                        <button class="button is-primary is-small" @click="openModal">Click here to proceed</button>
                      </div>
                    </div>
                  </a>
                </article>
                        ...
                        <div class="column is-12">
                          <div class="field">
                            ...
                            <template v-else-if="question.answer_type=== 5">
                              ...
                              <p style="margin-top: 10px"><small>Current: {{ question.response.document_url }}</small>
                                  <button type="button" @click="delete_response_file(question.id)" class="button is-small is-danger" style="margin-left: 15px">Delete</button></p>
                              </div>
...
<!--                          </div>-->
              ...
              <template v-else>
                <div class="field">
                  <a @click="finish_bid()" type="button" class="button is-primary is-small is-pulled-right">Submit</a>
                </div>
              </template>
            </div>
          </div>
  ...
  <div class="modal" id="ratios">
        ...
        <div class="modal-card">
          <header class="modal-card-head is-danger" style="padding: 5px 20px; background-color: #DBE9FE !important; font-size: 12px">
            <div class="columns">
          ...
          <form @submit.prevent="submit_ratios()" style="overflow: auto">
...
export default {
  ...
  methods:{
    ...
    async delete_response_file(question_id){
      try{
        let response = await supplier_tender.delete_response(question_id)
    ...
    async finish_bid(){
      try {
        let response = await supplier_tender.finish_technical_bid(this.$route.params.category_id)
        if (response.status === 200){
          window.toast.fire({icon: 'success', title: response.data['response_message']})
          this.$router.push(`/supplier/tender/category/financial/bid/${this.$route.params.category_id}`)
        }else{
    ...
    async submit_ratios(){
      try{
        ...
        let response = await supplier_tender.submit_ratios(this.active_section.id, this.form)
    ...
    async submit_profile_response(question){
      ...
      try{
        let response = await supplier_tender.submit_profile_response(data, question.id)
        if (response.status === 201){
    ...
    async submit_response(question){
      if (question.answer_type === 5){
        ...
        // .push(form_data)
        try{
          let response = await supplier_tender.bid(question.id, form_data)
          if (response.status === 201) {
        ...
        try{
          let response = await supplier_tender.bid(question.id, question_response)
          if (response.status === 201){
            window.toast.fire({icon: 'success', title: "Response saved"})
          }else{
            window.toast.fire({icon: 'error', title: response})
    ...
    async get_sections() {
      try {
        let response = await supplier_tender.bid_sections(this.$route.params.category_id)
    ...
    async get_sub_section_questions(section, sections) {
      ...
      try {
        let response = await supplier_tender.bid_questions(section.id)
    ...
    go_to_next_sub_section(){
      ...
      console.log(this.sub_section_data)
    ...
    go_to_next_section(){
      if (this.next_sub_section){
        ...
        // document.getElementById('button_'+this.next_section.id).scrollLeft = 0;
        n.click()
      }
    },
